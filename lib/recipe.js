...
function loader (mcVersion) {
  minorVersion = Number(mcVersion.split('.')[1])
...
function reformatShape (shape) {
  ...
  for (y = 0; y < shape.length; ++y) {
    row = shape[y]
    // post version 1.13, recipes are upside-down
    const index = minorVersion >= 13 ? shape.length - 1 - y : y
    out[index] = outRow = new Array(row.length)
